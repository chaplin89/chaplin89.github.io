...
Its structure is as follows:
 ...
 | | |
 -- | -- 
Analysis         | Is divided in subfolders. Every subfolder’s name is the ID of a resource (see below) Subfolders contains file related to my analysis, IDA DBs, tools I wrote and, eventually,  the resource itself decrypted (if the original is encrypted).
...
1. The third is the bytecode of program that can take the resource data, a path in input, and dump the resource in the path.
...
This driver is a ramdisk, its architecture is more or less the same that you’ll find as example in Microsoft website (see: [Analysis/0x0152_Drv.zip/RAMDISK](https://github.com/chaplin89/challenges/tree/master/ESET-2015/Analysis/0x0152_Drv.zip/RAMDISK)). You can read that code to understand the main functionality of this driver.
...
2. 6 is SP
3. 7 is begin of loaded program
...
1. Move data. Destination is a register.
1. (Native) Call. Make a call to the argument.
1. Call library. Arguments are the hash of the name of a (loaded) library and the hash of the name of a function exported by the library.
1. Push argument in the stack (load argument in memory pointed by SP and then decrement SP).
1. Pop dword from stack into a register (copy dword from memory pointed by SP into register then increment SP).
1. Logical operation, first operand is a register. Operation could be: not equal, equal, minor. If the condition is met, a flag is set.
...
1. Pop dword from stack into IP.
1. Arithmetic operation, first operand is a register. Operation could be add, sub, left shit, right shift, ROR, ROL, module.
1. Allocate memory. Operand is the size. Pointer to allocated memory is saved in Register 0.
1. Deallocate memory. Operand is the pointer. The program pointed by the argument is loaded in memory takes the place of the loaded one.
1. Terminate program. This is called when an invalid opcode is found.
...
It tries to refer a resource named “`resource0`” and there is no such resource in the assembly.
